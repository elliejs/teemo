...
class Teemo {
  ...
  Teemo._create(this._authKey, this._port, this._restClient, this._websocket): _subscriptions={};
  ...
  static Future<Teemo> create() async {
    ...
    Teemo teemo = Teemo._create(authKey, port, HttpClient(context: secCtx), await WebSocket.connect(
      'wss://127.0.0.1:$port',
      headers: {
      'Authorization': 'Basic ' + utf8.fuse(base64).encode('riot:$authKey')
      },
  ...
  void _startWebsocketListener() {
    this._websocket.listen((data) {
      ...
      List<EventSubscription> subscriptions = this._subscriptions[jsonData[1]] ?? [];
  ...
  void subscriptionFilterEndpoint(EventSubscription subscription, String endpoint, {Handler? behavior}) {
    behavior != null ? subscription.filterEndpoint(endpoint, behavior: behavior) : subscription.filterEndpoint(endpoint);
  }

  void subscriptionUnfilterEndpoint(EventSubscription subscription, String endpoint) {
  ...
  Future<JsonData> request(String method, String endpoint) async {
    ...
    switch (method) {
      case 'get':
      case 'GET':
        req = await this._restClient.getUrl(Uri.parse('https://127.0.0.1:${this._port}$endpoint'));
        break;
      default:
    ...
    req.headers.set(HttpHeaders.authorizationHeader, 'Basic ' + utf8.fuse(base64).encode('riot:${this._authKey}'));
